...
const DEFAULT_WINDOWS_STORE_ADDON_ID: &str = "";
...
impl LicenseConfig {
    fn from_app(app: &AppHandle) -> Self {
        let bundle_id = app.package_info().identifier.to_string();
...
pub fn purchase_unlock(app: AppHandle) -> Result<bool, String> {
    ...
    platform::purchase_unlock(&config)
}

#[tauri::command]
pub fn restore_purchases(app: AppHandle) -> Result<bool, String> {
    let config = LicenseConfig::from_app(&app);
    if config.dev_bypass_enabled {
        return Ok(true);
    }

    platform::restore_purchases(&config)
}
...
mod platform {
    ...
    pub fn check_status(config: &LicenseConfig) -> Result<LicenseStatus, String> {
        invoke_json(glance_macos_check_status, &MacBridgePayload::from_config(config))
    }

    pub fn purchase_unlock(config: &LicenseConfig) -> Result<bool, String> {
        ...
        let response: PurchaseResult =
            invoke_json(glance_macos_purchase_unlock, &MacBridgePayload::from_config(config))?;
        Ok(response.success)
    }

    pub fn restore_purchases(config: &LicenseConfig) -> Result<bool, String> {
        #[derive(Debug, serde::Deserialize)]
        #[serde(rename_all = "camelCase")]
    ...
    pub fn get_price_display(config: &LicenseConfig) -> Result<Option<String>, String> {
        ...
        let response: ProductResult =
            invoke_json(glance_macos_get_product, &MacBridgePayload::from_config(config))?;
        ...
        TPayload: Serialize,
    {
        ...
        if output_ptr.is_null() {
            return Err(String::from("Native StoreKit bridge returned an empty response"));
        }
...
mod platform {
    ...
    pub fn check_status(config: &LicenseConfig) -> Result<LicenseStatus, String> {
        ...
        if !record.purchased && store_reports_purchased(config)? {
    ...
    pub fn purchase_unlock(config: &LicenseConfig) -> Result<bool, String> {
        let purchase_result = request_purchase(config)?;
        if !purchase_result {
            return Ok(false);
        }
    ...
    pub fn restore_purchases(config: &LicenseConfig) -> Result<bool, String> {
        let purchased = store_reports_purchased(config)?;
        if !purchased {
            return Ok(false);
        }
    ...
    pub fn get_price_display(config: &LicenseConfig) -> Result<Option<String>, String> {
        ...
        let product_kinds = single_string_vector("Durable")?;
        let store_ids = single_string_vector(config.windows_store_addon_id.as_str())?;
        let result = context
        ...
        let products = result.Products().map_err(|error| error.to_string())?;
        if !products.HasKey(&HSTRING::from(config.windows_store_addon_id.as_str())).map_err(|error| error.to_string())? {
            return Ok(None);
        }

        let product = products
            .Lookup(&HSTRING::from(config.windows_store_addon_id.as_str()))
            .map_err(|error| error.to_string())?;
        let price = product.Price().map_err(|error| error.to_string())?;
        let formatted = price.FormattedPrice().map_err(|error| error.to_string())?;
    ...
    fn store_reports_purchased(config: &LicenseConfig) -> Result<bool, String> {
        let context = StoreContext::GetDefault().map_err(|error| error.to_string())?;
        let license = context
            .GetAppLicenseAsync()
            .map_err(|error| error.to_string())?
            .get()
            .map_err(|error| error.to_string())?;
        let add_on_licenses = license
            .AddOnLicenses()
            .map_err(|error| error.to_string())?;
        let addon_id = HSTRING::from(config.windows_store_addon_id.as_str());

        let has_key = add_on_licenses
            .HasKey(&addon_id)
            .map_err(|error| error.to_string())?;
        if !has_key {
            return Ok(false);
        ...
        let store_license = add_on_licenses
            .Lookup(&addon_id)
            .map_err(|error| error.to_string())?;
        store_license.IsActive().map_err(|error| error.to_string())
    }

    fn request_purchase(config: &LicenseConfig) -> Result<bool, String> {
        let context = StoreContext::GetDefault().map_err(|error| error.to_string())?;
        let result = context
            .RequestPurchaseAsync(&HSTRING::from(config.windows_store_addon_id.as_str()))
            .map_err(|error| error.to_string())?
            .get()
            .map_err(|error| error.to_string())?;
    ...
    fn save_record(config: &LicenseConfig, record: &RuntimeRecord) -> Result<(), String> {
        ...
        if status.0 != 0 {
            return Err(format!("Failed to write license registry value: {}", status.0));
        }
    ...
    fn single_string_vector(
        value: &str,
    ) -> Result<windows::Foundation::Collections::IVector<HSTRING>, String> {
        let vector = windows::Foundation::Collections::IVector::<HSTRING>::new()
            .map_err(|error| error.to_string())?;
        vector
            .Append(HSTRING::from(value))
            .map_err(|error| error.to_string())?;
        Ok(vector)
    }
}
...
mod platform {
    ...
    pub fn purchase_unlock(_config: &LicenseConfig) -> Result<bool, String> {
        Ok(true)
    }

    pub fn restore_purchases(_config: &LicenseConfig) -> Result<bool, String> {
        Ok(true)
    }
